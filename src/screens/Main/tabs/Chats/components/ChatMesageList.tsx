...
export const ChatMessageList = ({
  ...
  return (
    <ScrollToBottom
      ...
      className="chat-scroll-container chat-scroll flex-1 max-768px:max-h-[80vh] max-h-[70vh] px-5 pt-4 relative custom-scrollbar"
    >
        ...
        {messages.map((msg) => {
          return (
            <div
              key={msg.id}
              className={clsx(
                "w-fit max-w-[30rem] max-768px:py-2 max-768px:px-3 px-5 py-3 rounded-xl border",
                msg.fromMe
            ...
            >
              <div
                className="max-768px:text-[14px] dark:text-white/80 whitespace-pre-wrap break-words"
      ...
      {isTyping && (
        <div className="dark:bg-white/10 bg-gray-200 absolute left-3 bottom-5 flex items-center gap-1 p-3 rounded-full">
          <motion.span
            className="size-2 bg-main/40 rounded-full"
